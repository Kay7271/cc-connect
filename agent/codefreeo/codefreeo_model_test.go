...
func TestAgent_Name(t *testing.T) {
	a := &Agent{}
	if got := a.Name(); got != "codefree-o" {
		t.Errorf("Name() = %q, want %q", got, "codefree-o")
	}
}
...
// the model list produced by `codefree-o models` when it succeeds.
...
// provider-configured models when `codefree-o models` exits non-zero.
...
// literal string "codefree-o") is used when running the models sub-command.
...
// `codefree-o session delete <id>` and returns nil on success.
