...
type Agent struct {
	...
	model                string
	mode                 string
	cmd                  string // CLI binary name, default "codefree-o"
...
func opencodeProjectModelCachePath(dataDir, project string) string {
	...
	fileName := fmt.Sprintf("%s-%s.codefree-o-models.json", prefix, hex.EncodeToString(hash[:8]))
...
func (a *Agent) Name() string { return "codefree-o" }

func (a *Agent) SetWorkDir(dir string) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.workDir = dir
	slog.Info("codefree-o: work_dir changed", "work_dir", dir)
}
...
func (a *Agent) SetModel(model string) {
	...
	defer a.mu.Unlock()
	a.model = model
	slog.Info("codefree-o: model changed", "model", model)
}
...
func (a *Agent) startPersistentModelRefresh(snapshot opencodeModelDiscoverySnapshot, allowColdStart bool) {
	...
	go func() {
		...
		if err := a.storePersistentModelCache(snapshot, models); err != nil {
			slog.Warn("codefree-o: update persistent model cache failed", "path", snapshot.cachePath, "err", err)
		}
	}()
}
...
func (a *Agent) discoverModelsWithSnapshot(ctx context.Context, snapshot opencodeModelDiscoverySnapshot) []core.ModelOption {
	...
	out, err := c.Output()
	if err != nil {
		slog.Debug("codefree-o: discoverModels failed", "err", err)
		return nil
	}
	...
	models = normalizeModelOptions(models)
	if len(models) == 0 {
		slog.Debug("codefree-o: discoverModels: no models in output")
		return nil
	}
	return models
}
...
func (a *Agent) AvailableModels(ctx context.Context) []core.ModelOption {
	...
	if models := a.discoverModelsWithSnapshot(ctx, snapshot); len(models) > 0 {
		if err := a.storePersistentModelCache(snapshot, models); err != nil {
			slog.Warn("codefree-o: persist discovered model cache failed", "path", a.modelCachePath, "err", err)
		}
		return models
	}
...
// DeleteSession implements core.SessionDeleter via `codefree-o session delete <id>`.
func (a *Agent) DeleteSession(_ context.Context, sessionID string) error {
	...
	if out, err := c.CombinedOutput(); err != nil {
		return fmt.Errorf("codefree-o: delete session %s: %w: %s", sessionID, err, strings.TrimSpace(string(out)))
	}
	return nil
}
...
func (a *Agent) SetMode(mode string) {
	...
	slog.Info("codefree-o: mode changed", "mode", a.mode)
}
...
func (a *Agent) ProjectMemoryFile() string {
	...
	return filepath.Join(absDir, "CODEFREE-O.md")
}
...
func (a *Agent) GlobalMemoryFile() string {
	...
	return filepath.Join(homeDir, ".codefree-o", "CODEFREE-O.md")
}
...
func (a *Agent) SetActiveProvider(name string) bool {
	...
	if name == "" {
		a.activeIdx = -1
		slog.Info("codefree-o: provider cleared")
		return true
	}
	for i, p := range a.providers {
		if p.Name == name {
			a.activeIdx = i
			slog.Info("codefree-o: provider switched", "provider", name)
			return true
		}
	}
	return false
...
func querySessionMessageCounts() map[string]int {
	...
	if err != nil {
		slog.Warn("codefree-o: sqlite3 CLI not found, message counts unavailable", "err", err)
		return nil
	}
	...
	if err != nil {
		slog.Warn("codefree-o: sqlite3 query failed", "db_path", dbPath, "err", err)
		return nil
	}
...
func opencodeDBPath() string {
	...
	return filepath.Join(home, ".local", "share", "codefree-o", "codefree-o.db")
}
